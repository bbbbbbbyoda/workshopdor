...
from .models import Product, DetailImage, Order, OrderItem, Worker, OurService, Link, SocialMedia
...
def remove_from_cart(request, order_item_pk):
...
class ProductList(FilteredListView):
    ...
    def get_queryset(self):
        ...
        if query:
            query = query.lower()
            filtered_products = [product for product in all_products if query in product.name.lower()]
        else:
            filtered_products = all_products

        # Применение сортировки
        filtered_products.sort(key=lambda product: product.created_date)
